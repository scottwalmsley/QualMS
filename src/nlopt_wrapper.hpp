...
double nlopt_wrap(nlopt::opt &opt, std::vector<double> &x, Func_t func, this_type* this_ptr, Params& ...params) {
	using namespace std::placeholders;
	std::function<double(const std::vector<double>&, std::vector<double>&)> bind_f = bind(func, std::ref(*this_ptr),_1, _2, std::ref(params)...);
